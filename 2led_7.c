...
#define GPIOC_CLOCK RCC_APB2Periph_GPIOC
#define PORT_LED_CLOCK RCC_APB2Periph_GPIOA

uint16_t seg[10] = {0xC0, 0xF9, 0xA4, 0xB0, 0x99, 0x92, 0x82, 0xF8, 0x80, 0x90};
void Delay(uint32_t);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // for (uint8_t i = 0; i < 100; i += 2) // Chan
    //for (uint8_t i = 1; i < 100; i += 2) // Le
    // for (uint8_t i = 99; i >= 0; i--) // 99-0
    {
			if(i%2==0&&i%7==0){
      for (uint8_t j = 0; j < 30; j++)
      {
				// LED1
        GPIO_Write(GPIOC, seg[i / 10]); // Set value
        GPIO_SetBits(PORT_LED, LED1);   // Enable LED1
        Delay(1);
        GPIO_ResetBits(PORT_LED, LED1); // Disable LED1

        // LED2
        GPIO_Write(GPIOC, seg[i % 10]); // Set value
        GPIO_SetBits(PORT_LED, LED2);   // Enable LED2
        Delay(1);
        GPIO_ResetBits(PORT_LED, LED2); // Disable LED2
      }}
      // if (i == 0) i = 100; // 99-0
    }
  }
...
void GPIO_Config()
{
  ...
  RCC_APB2PeriphClockCmd(GPIOC_CLOCK | PORT_LED_CLOCK, ENABLE);
